...
// Initial positions for the game pieces
...
function handleMove(data, ws) {
    ...
    if (!piece || !isValidMove(piece, from, to)) {
        return; // Invalid move is silently ignored
    }

    // Check if the move is allowed
    if (!isMoveAllowed(from, to)) {
        return; // Move not allowed is silently ignored
    }
...
server.listen(5500, () => {
    ...
    // The line to open the browser is removed
});
